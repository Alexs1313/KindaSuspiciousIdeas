...
export type Verdict = 'suspicious' | 'not_suspicious';
...
const CasesContext =
  createContext<CasesState | null>(null);
...
export function CasesProvider({
  ...
  const [progressById, setProgressById] =
    useState<Record<string, CaseProgress>>(() =>
      buildDefaultProgress(cases),
    );
  ...
  const submitVerdict: CasesState['submitVerdict'] =
...
export function useCases() {
  ...
  if (!ctx) {
    throw new Error(
      'useCases must be used inside CasesProvider',
    );
  }
  return ctx;
}
